...
public class AESCipher {
    ...
    public static String encrypt(String data, String secretKey) {
        ...
        if(Strings.isBlank(secretKey) || secretKey.length() < 32)
            throw new BadRequestException("secret_key 는 최소 32 자 이상이어야 합니다.");
        ...
        try {
            ...
            throw new BadRequestException(String.format("AES 암호화 중 에러가 발생되었습니다. : %s", e.getMessage()));
        }
    }

    public static String decrypt(String data, String secretKey) throws BadRequestException {
        if(Strings.isBlank(data))
            throw new BadRequestException("AES 복호화 대상은 null 또는 빈문자열을 허용하지 않습니다.");
        if(Strings.isBlank(secretKey) || secretKey.length() < 32)
            throw new BadRequestException("secret_key 는 최소 32 자 이상이어야 합니다.");
        ...
        try {
            ...
            throw new BadRequestException(String.format("AES 복호화 중 에러가 발생되었습니다. : %s", e.getMessage()));
        }
    }
